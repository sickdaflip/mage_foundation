...
class Foundation_Sitemap_Model_Sitemap extends Mage_Sitemap_Model_Sitemap
{
    ...
    {
        $enabled = (bool) Mage::getStoreConfig('foundation_sitemap/general/enabled');
        if(!$enabled) {
            return parent::generateXml();
        }
        $helper = Mage::helper('foundation_sitemap');

        $limit = (int) Mage::getStoreConfig('foundation_sitemap/general/limit');
        if ($limit == 0) {
        ...
        try {
            ...
            Mage::getSingleton('adminhtml/session')->addError(
                $helper->__('Unable to delete old images sitemaps') . $e->getMessage()
            );
        }
