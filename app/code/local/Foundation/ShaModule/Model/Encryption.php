 ...
 */
class Foundation_ShaModule_Model_Encryption extends Mage_Core_Model_Encryption {
    /**
    ...
    {
        if (preg_match('#^\$[a-z0-9]{8}\$[a-z0-9]{40}$#i', $data)) {
            return sha1($data);
        }
        return md5($data);
    }
    ...
    {
        if (preg_match('#^\$[a-z0-9]{8}\$[a-z0-9]{40}$#i', $hash)) {
            $chunks = explode('$', $hash);
            if ($hash == "\${$chunks[1]}$" . sha1($chunks[1] . $password)) {
                return true;
            }
        }
